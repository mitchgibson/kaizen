...
import { createSidebarView, VIEW_TYPE_SIDEBAR } from './ui/sidebarView';
import { createFullPageView, VIEW_TYPE_FULL } from './ui/fullPageView';
import { KaizenSettingsTab } from './settings/settingsTab';
import {
  KaizenSettings,
  DEFAULT_SETTINGS,
...
export default class KaizenPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.registerView(
      VIEW_TYPE_SIDEBAR,
      (leaf) => new (createSidebarView(service))(leaf)
    );
    this.registerView(
      VIEW_TYPE_FULL,
      (leaf) => new (createFullPageView(service))(leaf)
    );
